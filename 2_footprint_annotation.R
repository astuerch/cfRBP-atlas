...
df1 <- result_df[, c(8, 10,11)]
colnames(df1) <- c("gene", "feature", "biotype")

result <- result_df %>%
  group_by(biotype) %>%
  summarise(unique_peaks = n_distinct(V1,V2))
...
cat("--- Number of fragments per biotype:\n")
print(result)


cat("--- total number of annotated fragments", sum(result$unique_peaks), "\n")
