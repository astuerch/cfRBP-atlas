...
cd /Volumes/My\ Book/analysis/RNA-seq_analysis/IGV_CLIP_analysis/
mkdir /Volumes/My\ Book/analysis/RNA-seq_analysis/IGV_CLIP_analysis/AGO1234/01.CM_differential_peaks
mkdir /Volumes/My\ Book/analysis/RNA-seq_analysis/IGV_CLIP_analysis/AGO1234/02.CM_differential_peaks_protected
mkdir /Volumes/My\ Book/analysis/RNA-seq_analysis/IGV_CLIP_analysis/AGO1234/03.fasta_selected_peaks


#sort bam files
#mkdir ./AGO1234
for d in ./AGO1234/*.bam
do
    NAME=$(basename $d .bam)
    samtools sort $d > ./AGO1234/${NAME}_sorted.bam
done
...
samtools merge ./AGO1234/AGO1234_KO_CM_smRNA_merged.bam ./AGO1234/*_CM_sorted.bam
samtools merge ./AGO1234/AGO1234_KO_Cell_smRNA_merged.bam ./AGO1234/*small_Cell_sorted.bam
samtools merge ./AGO1234/AGO1234_KO_Cell_longRNA_merged.bam ./AGO1234/*long_Cell_sorted.bam
...
bedtools bamtobed -i ./AGO1234/AGO1234_KO_Cell_smRNA_merged.bam | awk 'BEGIN{OFS="\t"} {key=$1"\t"$2"\t"$3"\t""AGO1234_KO_Cell_smRNA""\t"".""\t"$6} {coverage[key]+=1} END{for (i in coverage) if(coverage[i] > 4) print i, coverage[i]}'  | sortBed -i -  > ./AGO1234/AGO1234_KO_Cell_smRNA_merged_filtered.bed

bedtools bamtobed -i ./AGO1234/AGO1234_KO_CM_smRNA_merged.bam | awk 'BEGIN{OFS="\t"} {key=$1"\t"$2"\t"$3"\t""AGO1234_KO_CM_smRNA""\t"".""\t"$6} {coverage[key]+=1} END{for (i in coverage) if(coverage[i] > 9) print i, coverage[i]}'  | sortBed -i -  > ./AGO1234/AGO1234_KO_CM_smRNA_merged_filtered.bed

bedtools bamtobed -i ./AGO1234/AGO1234_KO_Cell_longRNA_merged.bam | awk 'BEGIN{OFS="\t"} {key=$1"\t"$2"\t"$3"\t""AGO1234_KO_Cell_longRNA""\t"".""\t"$6} {coverage[key]+=1} END{for (i in coverage) print i, coverage[i]}' | sortBed -i -  > ./AGO1234/AGO1234_KO_Cell_longRNA_merged_filtered.bed


rm ./AGO1234/AGO1234_KO_Cell_smRNA_merged.bed
rm ./AGO1234/AGO1234_KO_CM_smRNA_merged.bed
rm ./AGO1234/AGO1234_KO_Cell_longRNA_merged.bam


###prepare WT Cell data
#cluster reads with span of 10bp
bedtools cluster -i ./AGO1234/AGO1234_KO_Cell_longRNA_merged_filtered.bed -s -d 10 > ./AGO1234/AGO1234_KO_Cell_longRNA_merged_filtered_cluster.txt
bedtools cluster -i ./AGO1234/AGO1234_KO_Cell_smRNA_merged_filtered.bed -s -d 10 > ./AGO1234/AGO1234_KO_Cell_smRNA_merged_filtered_cluster.txt

#clean file: it combines together overlapping regions, filters for more than 3 reads
Rscript filtered_selected_peaks.R ./AGO1234/AGO1234_KO_Cell_longRNA_merged_filtered_cluster.txt ./AGO1234/AGO1234_KO_Cell_longRNA_merged_filtered.bed > ./AGO1234/AGO1234_KO_Cell_longRNA_merged_filtered.log 2>&1
Rscript filtered_selected_peaks.R ./AGO1234/AGO1234_KO_Cell_smRNA_merged_filtered_cluster.txt ./AGO1234/AGO1234_KO_Cell_smRNA_merged_filtered.bed > ./AGO1234/AGO1234_KO_Cell_smRNA_merged_filtered.log 2>&1

#sort files
sortBed -i ./AGO1234/AGO1234_KO_Cell_longRNA_merged_filtered.bed > ./AGO1234/AGO1234_KO_Cell_longRNA_merged_filtered_sorted.bed
sortBed -i ./AGO1234/AGO1234_KO_Cell_smRNA_merged_filtered.bed > ./AGO1234/AGO1234_KO_Cell_smRNA_merged_filtered_sorted.bed

rm ./AGO1234/AGO1234_KO_Cell_longRNA_merged_filtered_cluster.txt
rm ./AGO1234/AGO1234_KO_Cell_longRNA_merged_filtered.bed
rm ./AGO1234/AGO1234_KO_Cell_smRNA_merged_filtered_cluster.txt
rm ./AGO1234/AGO1234_KO_Cell_smRNA_merged_filtered.bed


#normalize coverage cell data using RPM - needed later to filter for cfRNA fragments that are not affected into cells
totalReadsKO1=$(awk '{sum+=$7} END {print sum}' ./AGO1234/AGO1234_KO_Cell_longRNA_merged_filtered_sorted.bed)
totalReadsKO2=$(awk '{sum+=$7} END {print sum}' ./AGO1234/AGO1234_KO_Cell_smRNA_merged_filtered.bed)

awk -v totalReadsWT="$totalReadsKO1" 'BEGIN{OFS=FS="\t"}{rpmWT = ($7 / totalReadsWT) * 1000000; print $1, $2, $3, $4, $5, $6, rpmWT}' ./AGO1234/AGO1234_KO_Cell_longRNA_merged_filtered_sorted.bed > ./AGO1234/AGO1234_KO_Cell_longRNA_merged_filtered_sorted_rpm.bed
awk -v totalReadsWT="$totalReadsKO2" 'BEGIN{OFS=FS="\t"}{rpmWT = ($7 / totalReadsWT) * 1000000; print $1, $2, $3, $4, $5, $6, rpmWT}' ./AGO1234/AGO1234_KO_Cell_smRNA_merged_filtered.bed > ./AGO1234/AGO1234_KO_Cell_smRNA_merged_filtered_rpm.bed
...
bedtools intersect -v -s  -sorted -bed -a WT_CM_smRNA_AGObatch_merged_filtered_reliable.bed -b ./AGO1234/AGO1234_KO_CM_smRNA_merged_filtered.bed > ./AGO1234/01.CM_differential_peaks/diff_WT_AGO1234_CM_smRNA.bed

wc -l ./AGO1234/01.CM_differential_peaks/diff_WT_AGO1234_CM_smRNA.bed
...
bedtools intersect -wb -s -f 0.5 -r -sorted -a WT_Cell_longRNA_AGObatch_merged_filtered_sorted_rpm.bed -b ./AGO1234/AGO1234_KO_Cell_longRNA_merged_filtered_sorted_rpm.bed | awk 'BEGIN{OFS=FS="\t"} $7/4 > $14' | cut -f1-6 | sortBed -i - | uniq  > ./AGO1234/02.CM_differential_peaks_protected/diff_exp_WT_AGO1234_Cell_longRNA.bed

bedtools intersect -wb -s -f 0.5 -r -sorted -a WT_Cell_smRNA_AGObatch_merged_filtered_sorted_rpm.bed -b ./AGO1234/AGO1234_KO_Cell_smRNA_merged_filtered_rpm.bed | awk 'BEGIN{OFS=FS="\t"} $7/4 > $14' | cut -f1-6 | sortBed -i - | uniq > ./AGO1234/02.CM_differential_peaks_protected/diff_exp_WT_AGO1234_Cell_smallRNA.bed
...
bedtools intersect -v -s -sorted -a ./AGO1234/01.CM_differential_peaks/diff_WT_AGO1234_CM_smRNA.bed -b ./AGO1234/02.CM_differential_peaks_protected/diff_exp_WT_AGO1234_Cell_longRNA.bed  ./AGO1234/02.CM_differential_peaks_protected/diff_exp_WT_AGO1234_Cell_smallRNA.bed > ./AGO1234/02.CM_differential_peaks_protected/AGO1234_CM_protected_peaks.bed
