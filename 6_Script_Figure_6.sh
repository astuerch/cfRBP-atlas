...
total_reads=$(samtools view -c TDP43_IP_CM_smRNA_merged_sorted.bam)
bedtools genomecov -ibam TDP43_IP_CM_smRNA_merged_sorted.bam -bg -scale $(echo "1000000 / $total_reads" | bc -l) > TDP43_IP_CM_smRNA_merged_sorted_RPM.bedgraph
/Users/alessandra/bedGraphToBigWig TDP43_IP_CM_smRNA_merged_sorted_RPM.bedgraph hg38.chrom.sizes.txt TDP43_IP_CM_smRNA_merged_sorted_RPM.bw

total_reads=$(samtools view -c IGF2BP3_IP_CM_smRNA_merged_sorted.bam)
bedtools genomecov -ibam IGF2BP3_IP_CM_smRNA_merged_sorted.bam -bg -scale $(echo "1000000 / $total_reads" | bc -l) > IGF2BP3_IP_CM_smRNA_merged_sorted_RPM.bedgraph
/Users/alessandra/bedGraphToBigWig IGF2BP3_IP_CM_smRNA_merged_sorted_RPM.bedgraph hg38.chrom.sizes.txt IGF2BP3_IP_CM_smRNA_merged_sorted_RPM.bw
...
#read lengths
awk '{print $3 - $2}' filtered_TDP43_IP_CM_smRNA_merged_sorted_MASTER_a.bed | sort | uniq -c > read_length_TDP43_a.txt

awk '{print $3 - $2}' /Volumes/AS_2023/scripts/eCLIP/bed_files/merged_cells/TARDBP.bed | sort | uniq -c > read_length_TDP43_eCLIP.txt

awk '{print $3 - $2}' filtered_IGF2BP3_IP_CM_smRNA_merged_sorted_MASTER_a.bed | sort | uniq -c > read_length_IGF2BP3_a.txt

awk '{print $3 - $2}' /Volumes/AS_2023/scripts/eCLIP/bed_files/merged_cells/IGF2BP3.bed | sort | uniq -c > read_length_IGF2BP3_eCLIP.txt


#to run in R
data <- read.table("/Users/alessandra/polybox/Shared/cfRBP_atlas_paper/data/from_fig6/RBP_IP/read_length_fragmentomics/read_length_TDP43_a.txt", quote="\"", comment.char="")

data2 <- read.table("/Users/alessandra/polybox/Shared/cfRBP_atlas_paper/data/from_fig6/RBP_IP/read_length_fragmentomics/read_length_IGF2BP3_a.txt", quote="\"", comment.char="")
...
data$sample <- "TDP43_IP"
data2$sample <- "IGF2BP3_IP"
data3$sample <- "Input_CM"

final <- rbind(data,data2, data3)

#normalization for sample
final$V1[final$sample == "TDP43_IP"] <- (final$V1[final$sample == "TDP43_IP"]/sum(final$V1[final$sample == "TDP43_IP"]))  *1000000
final$V1[final$sample == "Input_CM"] <- (final$V1[final$sample == "Input_CM"]/sum(final$V1[final$sample == "Input_CM"]))  *1000000
final$V1[final$sample == "IGF2BP3_IP"] <- (final$V1[final$sample == "IGF2BP3_IP"]/sum(final$V1[final$sample == "IGF2BP3_IP"]))  *1000000

desired_order <- c("Input_CM", "TDP43_IP", "IGF2BP3_IP")
...
colData_IGF <- subset(colData_batch2, genotype %in% c("WT", "IGF2BP3") & origin %in% c("CM", "IP") & treatment == "PNK")
colData_TDP <- subset(colData_batch2, genotype %in% c("WT", "TDP43") & origin %in% c("CM", "IP") & treatment == "PNK")
...
#results
res_igf <- results(ddsr_igf1, contrast=c("genotype", "IGF2BP3", "WT"))
res_tdp <- results(ddsr_tdp1, contrast=c("genotype", "TDP43", "WT"))
...
write.table(final_tot_counts_igf, "/Users/alessandra/polybox/Shared/cfRBP_atlas_paper/data/from_fig6/RBP_IP/deseq_analysis/deseq2_IGF2BP3_IP_vsInput_norm_counts.txt", quote = F, row.names = F, sep = "\t")

write.table(final_tot_counts_tdp, "/Users/alessandra/polybox/Shared/cfRBP_atlas_paper/data/from_fig6/RBP_IP/deseq_analysis/deseq2_TDP43_IP_vsInput_norm_counts.txt", quote = F, row.names = F, sep = "\t")
...
data_igf <- rpm %>%  mutate(across(starts_with("L0724"), as.numeric)) %>%
  mutate(input = log(rowMeans(select(., L0724A10, L0724A11, L0724A12), na.rm = TRUE)),
         IP = log(rowMeans(select(., L0724A18, L0724A19, L0724A20), na.rm = TRUE)), group = "IGF2BP3")

data_tdp <- rpm %>%  mutate(across(starts_with("L0724"), as.numeric))  %>%
  mutate(input = log(rowMeans(select(., L0724A10, L0724A11, L0724A12), na.rm = TRUE)),
         IP = log(rowMeans(select(., L0724A21, L0724A22, L0724A23), na.rm = TRUE)), group = "TDP43")
...
IGF2BP3_IP_genes <- read.table("/Users/alessandra/polybox/Shared/cfRBP_atlas_paper/data/from_fig6/RBP_IP/deseq_analysis/IGF2BP3_IP_DEGs.txt", quote="\"", comment.char="")
TDP43_IP_genes <- read.table("/Users/alessandra/polybox/Shared/cfRBP_atlas_paper/data/from_fig6/RBP_IP/deseq_analysis/TDP43_IP_DEGs.txt", quote="\"", comment.char="")
...
filtered_TDP43 <- anti_join(TDP43_IP_genes, IgG_IP_genes, by = "V1")
filtered_IGF2BP3 <- anti_join(IGF2BP3_IP_genes, IgG_IP_genes, by = "V1")

#format RNA gene names and biotypes
split_values <- strsplit(filtered_TDP43$V1, ":")
filtered_TDP43$biotype <- sapply(split_values, function(x) x[2])
filtered_TDP43$gene <- sapply(split_values, function(x) x[1])

split_values <- strsplit(filtered_IGF2BP3$V1, ":")
filtered_IGF2BP3$biotype <- sapply(split_values, function(x) x[2])
filtered_IGF2BP3$gene <- sapply(split_values, function(x) x[1])

#newly discovered fragments after IP (not present in input WT CM)
newdiscovery_TDP43 <- filtered_TDP43[!filtered_TDP43$V1 %in% wt_tdp$Geneid, , drop = FALSE] #733
newdiscovery_IGF2BP3 <-filtered_IGF2BP3[!filtered_IGF2BP3$V1 %in% wt_tdp$Geneid, , drop = FALSE] #852

#format RNA gene names and biotypes
split_values <- strsplit(newdiscovery_TDP43$V1, ":")
newdiscovery_TDP43$biotype <- sapply(split_values, function(x) x[2])
newdiscovery_TDP43$gene <- sapply(split_values, function(x) x[1])

split_values <- strsplit(newdiscovery_IGF2BP3$V1, ":")
newdiscovery_IGF2BP3$biotype <- sapply(split_values, function(x) x[2])
newdiscovery_IGF2BP3$gene <- sapply(split_values, function(x) x[1])

#extract bioype info to transfer in prism for plot
biotype_counts1 <- newdiscovery_TDP43 %>% group_by(biotype) %>% summarize(count = n())
biotype_counts2 <- newdiscovery_IGF2BP3 %>% group_by(biotype) %>% summarize(count = n())
