...
#combine replicates
sbatch --time=1:00:00 -n 4 --mem-per-cpu=2048 --tmp=40000  --wrap="bash -c 'samtools merge IGF2BP3_IP_CM_smRNA_merged.bam L0724A18_endogenousAlignments_genome_Aligned.out.bam L0724A19_endogenousAlignments_genome_Aligned.out.bam L0724A20_endogenousAlignments_genome_Aligned.out.bam'" #samtools merge with bam files only concatenates the input files without changing their coordinates

sbatch --time=1:00:00 -n 4 --mem-per-cpu=2048 --tmp=40000  --wrap="bash -c 'samtools merge TDP43_IP_CM_smRNA_merged.bam L0724A21_endogenousAlignments_genome_Aligned.out.bam L0724A22_endogenousAlignments_genome_Aligned.out.bam L0724A23_endogenousAlignments_genome_Aligned.out.bam'" #samtools merge with bam files only concatenates the input files without changing their coordinates
...
#sort merged files
sbatch --time=1:00:00 -n 4 --mem-per-cpu=2048 --tmp=40000  --wrap="bash -c 'samtools sort TDP43_IP_CM_smRNA_merged.bam > TDP43_IP_CM_smRNA_merged_sorted.bam'"

sbatch --time=1:00:00 -n 4 --mem-per-cpu=2048 --tmp=40000  --wrap="bash -c 'samtools sort IGF2BP3_IP_CM_smRNA_merged.bam > IGF2BP3_IP_CM_smRNA_merged_sorted.bam'"
...
bedtools bamtobed -i /Users/alessandra/polybox/sequencing/RNA-seq_analysis/RBP_IP_analysis/3-RBP_IP/IGV_plots/TDP43_IP_CM_smRNA_merged_sorted.bam > TDP43_IP_CM_smRNA_merged_sorted_readsID.bed
bedtools bamtobed -i /Users/alessandra/polybox/sequencing/RNA-seq_analysis/RBP_IP_analysis/3-RBP_IP/IGV_plots/IGF2BP3_IP_CM_smRNA_merged_sorted.bam > IGF2BP3_IP_CM_smRNA_merged_sorted_readsID.bed
...
split -l 10000000 IGF2BP3_IP_CM_smRNA_merged_sorted_readsID_2.bed IGF2BP3_chunk_
split -l 10000000 TDP43_IP_CM_smRNA_merged_sorted_readsID_2.bed TDP43_chunk_
split -l 10000000 IgG_IP_CM_smRNA_merged_sorted_readsID_2.bed IgG_chunk_

#for file in IGF2BP3_chunk_*; do
#    split -l 1000000 "$file" "${file}_second_"
#done

#intersection IP with IgG
for chunk in IGF2BP3_chunk_*; do
    NAME=$(basename $chunk)
...
cut -f4 intersect_TDP43_chunk_* | uniq > intersecting_reads_TDP43.bed
cut -f4 intersect_IGF2BP3_chunk_* | uniq > intersecting_reads_IGF2BP3.bed

#remove intersecting reads
awk '{print $4}' TDP43_IP_CM_smRNA_merged_sorted_readsID_2.bed > all_IDs_TDP43.txt
sbatch --time=4:00:00 -n 20 --mem-per-cpu=4096 --tmp=40000  --wrap="bash -c 'grep -Fvx -f intersecting_reads_TDP43.bed all_IDs_TDP43.txt > keep_reads_TDP43.txt'"

sbatch --time=4:00:00 -n 20 --mem-per-cpu=4096 --tmp=40000  --wrap="bash -c 'samtools view -h -N keep_reads_TDP43.txt TDP43_IP_CM_smRNA_merged_sorted.bam -o filtered_TDP43_IP_CM_smRNA_merged_sorted.bam'"


awk '{print $4}' IGF2BP3_IP_CM_smRNA_merged_sorted_readsID_2.bed > all_IDs_IGF2BP3.txt
sbatch --time=4:00:00 -n 20 --mem-per-cpu=4096 --tmp=40000  --wrap="bash -c 'grep -Fvx -f intersecting_reads_IGF2BP3.bed all_IDs_IGF2BP3.txt > keep_reads_IGF2BP3.txt'"

sbatch --time=4:00:00 -n 20 --mem-per-cpu=4096 --tmp=40000  --wrap="bash -c 'samtools view -h -N keep_reads_IGF2BP3.txt IGF2BP3_IP_CM_smRNA_merged_sorted.bam -o filtered_IGF2BP3_IP_CM_smRNA_merged_sorted.bam'"


#create bed files
bedtools bamtobed -i filtered_TDP43_IP_CM_smRNA_merged_sorted.bam | awk 'BEGIN{OFS="\t"} {key=$1"\t"$2"\t"$3"\t""TDP43_IP_smRNA""\t"".""\t"$6} {coverage[key]+=1} END{for (i in coverage) print i, coverage[i]}' | sortBed -i -  > filtered_TDP43_IP_CM_smRNA_merged_sorted.bed

bedtools bamtobed -i filtered_IGF2BP3_IP_CM_smRNA_merged_sorted.bam | awk 'BEGIN{OFS="\t"} {key=$1"\t"$2"\t"$3"\t""IGF2BP3_IP_smRNA""\t"".""\t"$6} {coverage[key]+=1} END{for (i in coverage) print i, coverage[i]}' | sortBed -i -  > filtered_IGF2BP3_IP_CM_smRNA_merged_sorted.bed
